...
es = elasticsearch.Elasticsearch(['http://{user}:{password}@{ep}'.format(user=es_user,
...
def login():
    ...
    if res['hits']['hits']:
        ...
        if hashed.encode() == bcrypt.hashpw(password, hashed.encode()):
            user = User()
            user.id = login
...
@app.route('/_query')
def query():
    query = flask.request.args.get('query', 0, type=str)
    parameter = flask.request.args.get('parameter', 0, type=str)
    # WORKS
    if parameter == 'all':
...
def get_by_id():
    index = flask.request.args.get('index', 0, type=str)
